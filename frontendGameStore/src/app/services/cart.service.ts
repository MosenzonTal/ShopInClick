...
/*Service is the way to communicate between different components*/
...
/*  Data Variable to store the cart information on the clients local storage*/
  ...
  private cartDataServer: CartModelServer = { /*default values assigned*/
    data: [{
      product: undefined,
      numInCart: 0
    }],
...
/*GET THE information from local Storage (If Any)*/
    ...
    /*CHECK IF THE INFO VARIABLE HAS SOME DATA IN IT*/
  ...
  AddProductToCart(id: number, quantity?: number) {
    this.productService.getSingleProduct(id).subscribe(prod => {
      ...
      // Cart is not empty
        ...
        // a. If chosen product is already in cart array => index is positive value
        if (index !== -1) {
